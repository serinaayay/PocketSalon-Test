...
import { View, Text, ScrollView, Image, Pressable, Dimensions, TouchableOpacity, Linking } from "react-native";
...
const ProductCard = ({ product, idx }: { product: Product; idx: number }) => {
  ...
  return (
    <View
      key={product.id || idx}
      className="bg-[#3F2305] rounded-xl mx-4 mt-2 mb-6 p-5 shadow-xl"
      style={{ minHeight: 200 }}>
      ...
      <View className="flex-row" style={{ alignItems: 'flex-start' }}>
        ...
        <View className="mr-4 items-center" style={{ width: 120 }}>
          ...
          {/* Tags/Badges below image */}
          <View className="items-center">
            {product.isNatural && (
              <View className="flex-row items-center bg-[#8B6B47] rounded-full px-2 py-1 mb-1">
                <Ionicons name="leaf-outline" size={12} color="#FFFFFF" style={{ marginRight: 4 }} />
                <Text className="text-white text-xs font-semibold">Natural</Text>
              </View>
            )}
            {product.isLocal && (
              <View className="flex-row items-center bg-[#8B6B47] rounded-full px-2 py-1">
                <Ionicons name="flag-outline" size={12} color="#FFFFFF" style={{ marginRight: 4 }} />
...
export default function HealthyHairGuide() {
  ...
  const filteredProducts = React.useMemo(() => {
    if (selectedCategory === "all") {
      // When "All" is selected, only show products with hairTypes (exclude hair damage only products)
    ...
    return sampleProducts.filter((product) => {
      // Only include products that explicitly match the selected hair type
      // Products without hairTypes are excluded when filtering by specific type
      ...
      // Exclude products without hairTypes when filtering by specific type
      return false;
    });
  }, [selectedCategory]);
